...
pub fn tokenize(text: &str) -> (Vec<Token>, Vec<SyntaxError>) {
    ...
    let mut offset: usize = rustc_lexer::strip_shebang(text)
        .map(|shebang_len| {
            tokens.push(Token { kind: SHEBANG, len: TextSize::try_from(shebang_len).unwrap() });
            shebang_len
        })
        .unwrap_or(0);
    ...
    for rustc_token in rustc_lexer::tokenize(text_without_shebang) {
        let token_len = TextSize::try_from(rustc_token.len).unwrap();
        let token_range = TextRange::at(TextSize::try_from(offset).unwrap(), token_len);
...
fn lex_first_token(text: &str) -> Option<(Token, Option<SyntaxError>)> {
    ...
    let token = Token { kind: syntax_kind, len: TextSize::try_from(rustc_token.len).unwrap() };
