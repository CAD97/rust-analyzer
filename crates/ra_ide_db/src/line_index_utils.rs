...
impl Iterator for OffsetStepIter<'_> {
    ...
    fn next(&mut self) -> Option<Step> {
        let (next, next_offset) = self
            ...
            .next()?;
        let next_idx = usize::from(next_offset - self.offset);
...
impl<'a> Edits<'a> {
    ...
    fn advance_edit(&mut self) {
        ...
        match self.edits.split_first() {
            Some((next, rest)) => {
                ...
                let diff = next.insert.len() as i64 - u32::from(next.delete.len()) as i64;
    ...
    fn translate(&self, x: TextSize) -> TextSize {
        if self.acc_diff == 0 {
            ...
            TextSize::from((u32::from(x) as i64 + self.acc_diff) as u32)
        }
    }
